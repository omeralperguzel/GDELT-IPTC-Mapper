...
def load_vargo_mapping(mapping_path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        print(f"⚠️  Vargo mapping not found at {mapping_path}")
        return pd.DataFrame()
...
def generate_embeddings(texts: list) -> np.ndarray:
    ...
    if not HAS_TRANSFORMERS:
        print("⚠️  Using random embeddings (sentence-transformers not installed)")
...
def reduce_dimensions(embeddings: np.ndarray) -> tuple:
    ...
    """
    if not HAS_SKLEARN:
        print("⚠️  Skipping PCA (scikit-learn not installed)")
...
def cluster_themes(X: np.ndarray, n_clusters: int = N_CLUSTERS) -> np.ndarray:
    ...
    """
    if not HAS_SKLEARN:
        print("⚠️  Skipping clustering (scikit-learn not installed)")
